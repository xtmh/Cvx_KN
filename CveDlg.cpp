	...
	for(x=0; x<PX; x++){
		for(y=0; y<PY ;y++){
			...
			if(m_bPeak){
				...
				if(m_bCal)	memcpy(&uSfc[x][y][0], &uClr[uFro[(PX-1)-x][(PZ-1)-m_nSldCore][y]][0], 3);	//	疑似ｶﾗｰ
				else		memset(&uSfc[x][y][0], uFro[(PX-1)-x][(PZ-1)-m_nSldCore][y], 3);			//	ﾓﾉｸﾛ
			}else{
				//	湾曲補正前ｽﾗｲｽ画像
				//if(m_bCal)	memcpy(&uSfc[x][y][0], &uClr[uFrm[x][m_nSldCore][y]][0], 3);		//	疑似ｶﾗｰ
				//else		memset(&uSfc[x][y][0], uFrm[x][m_nSldCore][y], 3);					//	ﾓﾉｸﾛ
				if(m_bCal)	memcpy(&uSfc[x][y][0], &uClr[uFrm[(PX-1)-x][(PZ-1)-m_nSldCore][y]][0], 3);	//	疑似ｶﾗｰ
				else		memset(&uSfc[x][y][0], uFrm[(PX-1)-x][(PZ-1)-m_nSldCore][y], 3);			//	ﾓﾉｸﾛ
			}
		}
	}
	Invalidate(FALSE);
