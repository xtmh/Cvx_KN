	...
	else
	{
		...
		if(m_bPeak){
			...
			for(x=10; x<10+PY/EXPND; x++){
				nx = (x-10)*EXPND;
				y = IMG_OFS+pkDepth[nx][m_nSld].nOrg;	//	XY軸交換,Y軸反転
...
void CCveDlg::fileOpen()
{
	...
	if(dlg.DoModal() == IDOK){
		...
		if(bRef){
			...
			for(int x=0; x<PX; x++){
				for(int y=0; y<PY; y++){
					pkTmp[y][x].dSub*=-1;	//	ﾘﾌｧﾚﾝｽﾃﾞｰﾀは極性反転
				}
			}
			//	圧縮
			ret = LZ4_compress((char*)pkTmp, (char*)pkRef, sizeof(CPeak)*PX*PY);	//	pkDepth -> pkRef(圧縮)	
			if(ret<=0){
			...
			imgMeas2();		//	試料計測				⑧dSmp <- ⑥dCal, ⑤dSub	<< ⑥dCal <- ②dCrv, ③dAvg
			//imgIncl();	//	傾斜面補正値(dInc)算出	dInc <- dSmp	※.面画像生成後に移動
		}
		imgCalc();			//	dSubによる湾曲補正処理		uFro <- uFrm <- dSub
	...
	imgSlc();			//	X-Y断面生成		uSfc <- nOrg or uFro or uFrm <- nOrg
...
#pragma omp parallel for
		for(x=0; x<PX; x++){
			nMax[x] = 0;
			pkDepth[y][x].nOrg = 510;		//	510で開始する(特別な値として)
			for(z=0; z<PZ; z++){
...
//#pragma omp parallel for
	...
	x = 100;
	for(y=0; y<PY; y++){
		//s.Format("%d,%d,%.3f,%.3f\n", y, pkDepth[y][x].nOrg, pkDepth[y][x].dCrv, pkDepth[y][x].dSub);		//	X軸ﾌﾟﾛﾌｧｲﾙ
		s.Format("%d,%d,%.3f,%.3f\n", y, pkDepth[x][y].nOrg, pkDepth[x][y].dCrv, pkDepth[x][y].dSub);		//	Y軸ﾌﾟﾛﾌｧｲﾙ
		//pkDepth[x][y].dSub = 0;	//	Y軸に黒線
...
void CCveDlg::imgLwSub()
{
	...
	for(y=0; y<PY; y++){
		for(x=0; x<PX; x++){
			...
			if((dMin > pkDepth[y][x].dAvg)&&(pkDepth[y][x].dAvg>0)){		//	負値を許容しない
...
#pragma omp parallel for
	for(y=0; y<PY; y++){
		for(z=0; z<PZ; z++){
			if(m_bCal){
				//	疑似ｶﾗｰ表示
				if(m_bCurv)	memcpy(&uDsp[z][y][0], &uClr[uFro[m_nSld][z][y]][0], 3);	//	湾曲補正画像
				else		memcpy(&uDsp[z][y][0], &uClr[uFrm[m_nSld][z][y]][0], 3);	//	無補正画像
			}else{
				//	ｸﾞﾚｰｽｹｰﾙ表示
				if(m_bCurv)	memset(&uDsp[z][y][0], uFro[m_nSld][z][y], 3);	//	湾曲補正画像
				else		memset(&uDsp[z][y][0], uFrm[m_nSld][z][y], 3);	//	無補正画像
			}
		}
	}
	Invalidate(FALSE);
	...
	for(x=0; x<PX; x++){
		for(y=0; y<PY ;y++){
			...
			if(m_bPeak){
				...
				if(m_bCal)	memcpy(&uSfc[x][y][0], &uClr[255-(uchar)pkDepth[y][(PX-1)-x].nOrg][0], 3);	//	疑似ｶﾗｰ
				else		memset(&uSfc[x][y][0], (255-(uchar)pkDepth[y][(PX-1)-x].nOrg), 3);			//	ﾓﾉｸﾛ
			}else if(m_bCurv){
				//	湾曲補正後ｽﾗｲｽ画像
				//if(m_bCal)	memcpy(&uSfc[x][y][0], &uClr[uFro[x][m_nSldCore][y]][0], 3);		//	疑似ｶﾗｰ
				//else		memset(&uSfc[x][y][0], uFro[x][m_nSldCore][y], 3);						//	ﾓﾉｸﾛ
				if(m_bCal)	memcpy(&uSfc[x][y][0], &uClr[uFro[(PX-1)-x][m_nSldCore][y]][0], 3);		//	Z反転	疑似ｶﾗｰ
				else		memset(&uSfc[x][y][0], uFro[(PX-1)-x][m_nSldCore][y], 3);				//			ﾓﾉｸﾛ
			}else{
				//	湾曲補正前ｽﾗｲｽ画像
				//if(m_bCal)	memcpy(&uSfc[x][y][0], &uClr[uFrm[x][m_nSldCore][y]][0], 3);		//	疑似ｶﾗｰ
				//else		memset(&uSfc[x][y][0], uFrm[x][m_nSldCore][y], 3);					//	ﾓﾉｸﾛ
				if(m_bCal)	memcpy(&uSfc[x][y][0], &uClr[uFrm[(PX-1)-x][m_nSldCore][y]][0], 3);	//	疑似ｶﾗｰ
				else		memset(&uSfc[x][y][0], uFrm[(PX-1)-x][m_nSldCore][y], 3);			//	ﾓﾉｸﾛ
			}
		}
	}
	Invalidate(FALSE);
...
void CCveDlg::imgFlat()
{
	...
	for(y=0; y<PY; y++){
		for(x=0; x<PX; x++){
			if(uBin[x][y] != 0){
				//	ﾜｰｸｴﾘｱ
				ftSfc.dAvg += uSfc[x][y][RED];	//	画素値積算
				n++;
			}
		}

	}
	ftSfc.dAvg /= (double)n;	//	平均画素値
	//	平均分散値, Ry, Ra算出
	ftSfc.dDist = 0.0;
	for(y=0; y<PY; y++){
		for(x=0; x<PX; x++){
			if(uBin[x][y] != 0){
				dSum = (uSfc[x][y][RED]-ftSfc.dAvg);	//	偏差
				ftSfc.dRa += fabs(dSum);				//	累積偏差				
				ftSfc.dDist += (dSum*dSum);				//	累積分散値
				if(dSum>ftSfc.dMax)
					ftSfc.dMax = dSum;
